...
namespace AndroidHUD
{
    public class AndHUD
    {
        ...
        int previousContextKey = 0; //used to identify the last known context used in the current dialog
        ...
        void showStatus (Context context, bool spinner, string status = null, MaskType maskType = MaskType.Black, TimeSpan? timeout = null, Action clickCallback = null, bool centered = true, Action cancelCallback = null)
        {
            ...
            previousContextKey = context.GetHashCode ();
        ...
        void DismissCurrent (Context context = null)
        {
            lock (dialogLock) {
                if (CurrentDialog != null) {
                    int contextKey = context.GetHashCode ();
                    waitDismiss.Set ();

                    Action actionDismiss = () => {
                        CurrentDialog.Hide ();

                        //the previous context and the current context must match in order to dismiss the current dialog;
                        //otherwise Java.Lang.IllegalArgumentException is thrown because the curent dialog is not associated 
                        ...
                        //might not be necessary but here as a safety measure to prevent memory leaks caused by 
                        //dialogs that are not associated with current window.
                        if (previousContextKey != contextKey)
                            GC.Collect ();
